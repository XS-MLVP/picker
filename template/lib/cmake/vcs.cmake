if(SIMULATOR STREQUAL "vcs")
	...
	if(${COVERAGE} STREQUAL "ON")
		...
		if(NOT VCS_HAS_COVERAGE_FLAGS)
			set(SIMULATOR_FLAGS "${SIMULATOR_FLAGS};-cm;line+cond+fsm+tgl+branch")
		endif()
		if(NOT "$ENV{VCS_COVERAGE_DB}" STREQUAL "")
			get_filename_component(VCS_COVERAGE_PARENT "$ENV{VCS_COVERAGE_DB}" DIRECTORY)
			file(MAKE_DIRECTORY "${VCS_COVERAGE_PARENT}")
			set(SIMULATOR_FLAGS "${SIMULATOR_FLAGS};-cm_dir;$ENV{VCS_COVERAGE_DB}")
		elseif(NOT VCS_HAS_COVERAGE_DIR)
		...
		if(EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/coverage/coverage.cpp")
			...
			target_include_directories(coverage_helper PRIVATE ${VCS_HOME}/include)
			target_link_libraries(coverage_helper PRIVATE ${VCS_HOME}/linux64/lib/libucapi.so dl m pthread)
			set_target_properties(coverage_helper PROPERTIES
				OUTPUT_NAME coverage
				RUNTIME_OUTPUT_DIRECTORY "${CMAKE_CURRENT_SOURCE_DIR}/coverage"
				BUILD_RPATH "${VCS_HOME}/linux64/lib")
		endif()
	endif()
