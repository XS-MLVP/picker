...
namespace picker { namespace parser {
    ...
    namespace {
        ...
        void append_direct_filelist_entry(const std::string &entry, std::vector<std::string> &out)
        {
            ...
            if (!std::filesystem::exists(fs_path)) {
                // Keep compatibility with the previous search-oriented behavior:
                // missing direct filelist entries are ignored by the parser.
                return;
            }
    ...
    int parse_sv_with_slang(const picker::export_opts &opts, const std::map<std::string, int> &requested_module_counts,
                            ...
                            std::vector<picker::sv_module_define> &sv_module_result)
    {
        ...
        if (has_explicit_requests) {
            for (const auto &[module_name, _] : requested_module_counts) { resolved_module_names.push_back(module_name); }
        } else {
