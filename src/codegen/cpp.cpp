...
namespace mcv { namespace codegen {
    int gen_dpi(std::vector<sv_pin_member> pin, std::string &src_module_name,
                ...
                std::string &simulator, std::string &vflag)
    {
        ...
        std::string logic_pin_declaration, wire_pin_declaration, pin_connect,
            dpi_function_export, dpi_function_implement, hpp_logic_annotation,
            hpp_input_pins, hpp_output_pins, cpp_reinit_pins, cpp_bind_pins,
            cpp_add_ports;

        // DPI Segment Templates
        std::string pin_connect_template =
            "    .{{logic_pin}}({{logic_pin}}),\n";
        std::string logic_pin_declaration_template =
            "  logic {{logic_pin_length}} {{logic_pin}};\n";
        std::string wire_pin_declaration_template =
            "  wire {{logic_pin_length}} {{logic_pin}};\n";

        std::string dpi_function_export_template =
            "  export \"DPI-C\" function get_{{logic_pin}};\n"
            "  export \"DPI-C\" function set_{{logic_pin}};\n";

        std::string dpi_function_implement_template =
            "  function get_{{logic_pin}};\n"
        ...
        std::string hpp_logic_annotation_template =
            "    // {{logic_pin_type}} {{logic_pin_length}} {{logic_pin}}\n";
        std::string hpp_logic_pin_template = "    XData {{logic_pin}};\n";
        std::string cpp_reinit_pins_template =
            "    this->{{logic_pin}}.ReInit({{logic_pin_length}}, IOType::{{logic_pin_type}}, \"{{logic_pin}}\");\n";
        std::string cpp_bind_pins_template =
            "    this->{{logic_pin}}.BindDPIRW(get_{{logic_pin}}, set_{{logic_pin}});\n";
        std::string cpp_add_ports_template =
            "    this->port.Add(this->{{logic_pin}}.mName, this->{{logic_pin}});\n";
        ...
        for (int i = 0; i < pin.size(); i++) {
            ...
            cpp_reinit_pins =
                cpp_reinit_pins + env.render(cpp_reinit_pins_template, data);
            cpp_bind_pins =
                cpp_bind_pins + env.render(cpp_bind_pins_template, data);
            cpp_add_ports =
                cpp_add_ports + env.render(cpp_add_ports_template, data);

            if (pin[i].logic_pin_type == "Input")
                hpp_input_pins =
                    hpp_input_pins + env.render(hpp_logic_pin_template, data);
            else if (pin[i].logic_pin_type == "Output")
                hpp_output_pins =
                    hpp_output_pins + env.render(hpp_logic_pin_template, data);

            if (pin[i].logic_pin_length == 0)
                data["logic_pin_length"] = "";
            else
                data["logic_pin_length"] =
                    "[" + std::to_string(pin[i].logic_pin_length - 1) + ":"
                    + "0" + "]";
            hpp_logic_annotation =
                hpp_logic_annotation
                + env.render(hpp_logic_annotation_template, data);
            logic_pin_declaration =
                logic_pin_declaration
                + env.render(logic_pin_declaration_template, data);
            wire_pin_declaration =
                wire_pin_declaration
                + env.render(wire_pin_declaration_template, data);
            dpi_function_export =
                dpi_function_export
                + env.render(dpi_function_export_template, data);
            dpi_function_implement =
                dpi_function_implement
                + env.render(dpi_function_implement_template, data);
        }
        // remove extra ','
        ...
        data["__ADD_PORTS__"]        = cpp_add_ports;
        ...
        data["__VERILATOR_TRACE__"] = verilator_trace;
    ...
    void cpp(cxxopts::ParseResult opts, std::vector<sv_pin_member> pin)
    {
        ...
        gen_dpi(pin, src_module_name, dst_module_name, src_dir, dst_dir,
                wave_file_name, simulator, vflag);
        // copy verilog source
