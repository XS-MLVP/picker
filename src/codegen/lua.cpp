...
namespace picker { namespace codegen {
    ...
    void lua(picker::export_opts &opts, std::vector<picker::sv_signal_define> external_pin,
                std::vector<picker::sv_signal_define> internal_signal, nlohmann::json &signal_tree_json)
    {
        //
        std::string src_dir         = opts.source_dir + "/lua";
        std::string dst_dir         = opts.target_dir + "/lua";
        ...
        lua_scope::render_external_pin(external_pin, xdata_init, xdata_bindrw, xport_add, swig_constant);
        // Generate Internal Signal
        lua_scope::render_internal_signal(internal_signal, xdata_init, xdata_bindrw, xport_add, swig_constant);
        // Generate Cascaded Porst
        lua_scope::render_cascaded_signals("", signal_tree_json, cascaded_signals);
        ...
        auto lua_location = picker::get_xcomm_lib("lua", erro_message);
        if (lua_location.empty()) { PK_FATAL("%s\n", erro_message.c_str()); }
