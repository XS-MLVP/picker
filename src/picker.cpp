...
int main(int argc, char **argv)
{
    ...
    if (main_opts.check) {
        MESSAGE("[OK ] Version: %s-%s-%s%s", PROJECT_VERSION, GIT_BRANCH,
                GIT_HASH, GIT_DIRTY);
        ...
        for (auto lang : check_langs) {
            ...
            if (lib_location.size() == 0) {
                ERROR("[Err] Support %6s (find: '%s' fail)", lang,
                      check_libs[i]);
            } else {
                MESSAGE("[OK ] Support %6s (find: '%s' success)", lang,
                        lib_location.c_str());
            }
            i += 1;
        }
    ...
    if (export_opts.language.size() != 0) {
        ...
        if (lib_location.size() == 0) {
            ERROR("[Err] Support %6s (find: '%s' fail)",
